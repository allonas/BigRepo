...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.dataLoader = [[ROSingleDataLoader alloc] initWithDatasource:(NSObject<RODatasource> *)[[DatasourceManager sharedInstance] mapDSDS]
...
- (void)viewDidLoad {
    ...
    [[[ROUtils sharedInstance] analytics] logPage:@"MapDetail"];
...
- (void)loadDataSuccess:(MapDSDSItem *)item {
    [super loadDataSuccess:item];

    self.navigationItem.title = item.town;
    
	self.textLabel1.text = item.enlaceGoogleMaps;

	self.textLabel2.text = [item.location stringValue];
