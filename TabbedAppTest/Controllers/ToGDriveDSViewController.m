...
@interface ToGDriveDSViewController ()
...
@implementation ToGDriveDSViewController
...
- (void)viewDidLoad {
    ...
    [[[ROUtils sharedInstance] analytics] logPage:@"ToGDriveDS"];

    self.navigationItem.title = NSLocalizedString(@"ToGDriveDS", nil);
    ...
    [self.behaviors addObject:[ROCreateItemBehavior behaviorViewController:self crudControllerClass:[ToGDriveDSFormViewController class]]];
...
- (void)configureCell:(ROTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.titleLabel.text = item.owner;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SearchDSDSItem *item = self.items[(NSUInteger)indexPath.row];
    GDListViewController *destinationViewController = [GDListViewController new];
