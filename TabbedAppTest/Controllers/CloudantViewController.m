...
@interface CloudantViewController ()
...
@implementation CloudantViewController
...
- (void)viewDidLoad {
    ...
    [[[ROUtils sharedInstance] analytics] logPage:@"Cloudant"];
    
    [self.optionsFilter addDesignFilter:[ROStringFilter filterWithFieldName:kCloudantDSSchemaItemSalesPerson
                                                                      value:self.searchDSDSItem.salesPerson]];

    self.navigationItem.title = NSLocalizedString(@"Cloudant", nil);
    ...
    [self.behaviors addObject:[ROCreateItemBehavior behaviorViewController:self crudControllerClass:[CloudantFormViewController class]]];
...
- (ROOptionsFilter *)optionsFilter {
    ...
    if (!_optionsFilter) {
        ...
        _optionsFilter.sortColumn = kCloudantDSSchemaItemSalesPerson;
...
- (void)configureCell:(ROTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.titleLabel.text = item.salesPerson;
    cell.detailLabel.text = item.salesNumber;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CloudantDSSchemaItem *item = self.items[(NSUInteger)indexPath.row];
    CloudantDetailViewController *destinationViewController = [CloudantDetailViewController new];
