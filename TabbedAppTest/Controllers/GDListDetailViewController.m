...
@interface GDListDetailViewController ()
...
@implementation GDListDetailViewController
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.dataLoader = [[ROSingleDataLoader alloc] initWithDatasource:(NSObject<RODatasource> *)[[DatasourceManager sharedInstance] googleDriveDS]
...
- (void)viewDidLoad {
    ...
    [[[ROUtils sharedInstance] analytics] logPage:@"GDListDetail"];
...
- (void)loadDataSuccess:(GoogleDriveDSSchemaItem *)item {
    [super loadDataSuccess:item];

    
	self.textLabel1.text = item.title;

	self.textLabel2.text = item.createdBy;

	self.textLabel3.text = [item.modifiedDate ro_datetimeStringValue];

	self.textLabel4.text = item.downloadUrl;
